...
x = np.loadtxt(open('./speed_region.csv', "rb"), delimiter=",", skiprows=0)
...
########################################################################################
path2 = './speed_city.csv'
speed = np.loadtxt(open(path2, "rb"), delimiter=",", skiprows=0).reshape(-1, opt.seq_len, city_size, city_size)
...
mean_spd = np.mean(spd, axis=0)     # mean_spd shape: (seq_len, region_length, region_width)
vmax = mean_spd.max()    # max number of speed in the region
mean_spd = np.where(mean_spd > 1, mean_spd, 0)

np.savetxt('./CurbGAN_test/real_region_mean_' + str(opt.region_i) + '_' + str(opt.region_j) + '.csv', mean_spd.reshape(-1, region_width), delimiter=',')
...
######################################################################################
# prepare noise
batch_x = torch.randn(spd.shape[0], opt.seq_len * opt.init_dim * region_width * region_length)
...
all_adj = np.loadtxt(open('./all_region_speed_adjacency.csv', "rb"), delimiter=",", skiprows=0)
...
batch_adj = np.tile(current_region_adj.reshape(opt.seq_len, region_width * region_length, region_width * region_length), (spd.shape[0], 1, 1))
...
y1 = torch.zeros(spd.shape[0] * opt.seq_len * region_width * region_length, 1) + region_i
y2 = torch.zeros(spd.shape[0] * opt.seq_len * region_width * region_length, 1) + region_j
...
y3 = np.tile(y3, (spd.shape[0], 1, 1)).reshape(spd.shape[0] * opt.seq_len * region_width * region_length, 1)
...
y4 = np.tile(y4, (spd.shape[0], 1, 1)).reshape(spd.shape[0] * opt.seq_len * region_width * region_length, 1)
y4 = torch.from_numpy(y4).float()
batch_y = torch.cat((batch_y, y4), 1).view(spd.shape[0], opt.seq_len, region_width * region_length, -1)
...
###################### RMSE ###############################
mean_to_mean_dist = mean_spd - mean_reg
RMSE_ls = []
...
###################### MAPE ###############################
mean_to_mean_dist = np.absolute(mean_spd - mean_reg).reshape(-1, region_width)  # shape:(seq_len, region_length, region_width)
percent = mean_to_mean_dist / (mean_spd.reshape(-1, region_width))
